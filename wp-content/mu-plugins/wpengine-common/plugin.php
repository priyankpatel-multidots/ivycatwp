...
class WpeCommon extends WpePlugin_common {
    ...
    public static function http_to_varnish( $method = 'PURGE', $hostname = null, $headers = [] ) {
        ...
        foreach ( $wpe_varnish_servers as $varnish_server ) {
            $res = WpeCommon::http_request_async( $method, $varnish_server, $varnish_port, $hostname, $varnish_endpoint, $headers, 0 );
            if ( false === $res ) {
...
function wpe_graphql_purge_cb( $purge_keys ) {
    ...
    $res = WpeCommon::http_to_varnish( 'PURGE_GRAPHQL', $hostname, [ 'X-GraphQL-Purge-Keys' => $purge_keys ] );
    if ( is_wp_error( $res ) ) {
        foreach ( $res->get_error_messages() as $error_message ) {
